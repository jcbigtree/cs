...
# Authors: Jinchao Liu liujinchao2000@gmail.com
...
if __name__ == '__main__':
    ...
    n_variables = 512    
    n_constraints = 120
    A, b, x_opt = random_min_l1_with_equality_constraints(n_variables=n_variables, 
                                                          n_constraints=n_constraints,
                                                          sparsity_level=0.039)
       
    #x_recovered = l1_min_with_equality_constraints(A, b)
    
    l1equ_solver = L1EqualitySolver(A=A, b=b,
                                    initial_solution=[],
                                    tolerance=1e-3,
                                    max_iters=50,
                                    enable_staging=True,
    ...
    x_recovered = l1equ_solver.solve()
    staged_residual = l1equ_solver.staged_residual
    
    res = x_recovered - x_opt
    print np.dot(res,res)
            
    import matplotlib.pyplot as plt
    plt.figure()
    plt.subplot(3,1,1)
    plt.plot(staged_residual,'g',marker='o',linewidth='2')
    plt.ylabel('KKT residual')

    plt.subplot(3,1,2)
    plt.ylabel('Surrogate duality gap')
    plt.plot(l1equ_solver.staged_sdg,'g',marker='o',linewidth='2')
    
    plt.subplot(3,1,3)
    plt.xlabel('Iteration')
    plt.ylabel('Step size')
    plt.plot(l1equ_solver.staged_step_size,'g',marker='o',linewidth='2')
    ...
    plt.figure()
    plt.subplot(2,1,1)
    plt.plot(x_opt,'g',marker='+', label='original')
    plt.title('Ground Truth')
    plt.xlim(0,n_variables)
    
    plt.subplot(2,1,2)
    plt.plot(x_recovered,'b',marker='+', label='recovered')
    plt.title('Recovered')
    plt.xlim(0,n_variables)
    plt.show()
